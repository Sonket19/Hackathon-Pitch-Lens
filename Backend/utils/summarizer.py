...
class GeminiSummarizer:
    ...
    def __init__(self) -> None:
        ...
        self._refresh_grounding_handles()
        try:
    ...
    def _generate_text(self, prompt: str, media_parts: Optional[List[Part]] = None) -> str:
        ...
        content: Union[str, List[Union[str, Part]]] = _build_content(media_parts)
        attempt_media = bool(media_parts)
        ...
        while True:
            try:
                ...
                message = str(exc)
                if self._maybe_disable_google_search(message):
                    content = _build_content(media_parts) if attempt_media else prompt
                    continue
    ...
    def _refresh_grounding_handles(self) -> None:
        self._grounding_tools = self._grounding_agent.tools
        self._grounding_tool_config = self._grounding_agent.tool_config

    def _maybe_disable_google_search(self, message: str) -> bool:
        normalized = message.lower()
        if "google_search_retrieval" not in normalized and "google_search" not in normalized:
            return False

        disabled = self._grounding_agent.disable_google_search()
        if disabled:
            logger.warning(
                "Disabling Google Search grounding due to API rejection: %s",
                message,
            )
            self._refresh_grounding_handles()
        return disabled
