...
class GeminiSummarizer:
    ...
    async def generate_memo(self, deal_data: Dict[str, Any], weightage: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                memo_payload: Dict[str, Any] = json.loads(clean) if clean else {}
                highlights = extract_financial_highlights(deal_data.get("extracted_text", {}))
                if highlights:
                    backfill_financials_section(memo_payload, highlights)
                return memo_payload
            except json.JSONDecodeError:
