...
class GroundedKnowledgeAgent:
    ...
    def __init__(
        ...
        *,
        ...
        tools: List[Tool] = []
        self._google_search_tool: Optional[Tool] = None
        ...
        if use_google:
            try:
                ...
                self._google_search_tool = Tool.from_google_search_retrieval(
                    google_search
                )
    ...
    # ------------------------------------------------------------------
    def disable_google_search(self) -> bool:
        """Remove the Google Search tool when the API rejects it."""

        if not self._google_search_tool:
            return False

        if self._tools:
            self._tools = [
                tool for tool in self._tools if tool is not self._google_search_tool
            ] or None
