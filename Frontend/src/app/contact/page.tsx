...
import { Loader2, Mail, Phone, Calendar as CalendarIcon } from 'lucide-react';
...
const buildFounderDirectory = (analysisData: AnalysisData): FounderContact[] => {
  ...
  const memoFounders = analysisData.memo?.draft_v1?.company_overview?.founders as
    | Founder[]
    | undefined;
...
export default function ContactDirectoryPage() {
  ...
  const directory: CompanyDirectory[] = useMemo(() => {
    return startups.map(startup => ({
      companyName:
        normaliseString(startup.metadata?.display_name) ||
        normaliseString(startup.metadata?.company_name) ||
        'Unknown Company',
      founders: buildFounderDirectory(startup),
    }));
  }, [startups]);
  ...
  const foundersWithEmails = useMemo(
    () =>
      directory
        .flatMap(company => company.founders)
        .filter(founder => founder.emails.length > 0),
    [directory]
  );
  ...
  const handleSchedule = () => {
    ...
    const uniqueCompanies = Array.from(new Set(selected.map(founder => founder.companyName).filter(Boolean)));
    const companyText = uniqueCompanies.length === 1 ? uniqueCompanies[0] : uniqueCompanies.join(' & ');

    const formattedDate = format(selectedDate, 'MMMM d, yyyy');
    const formattedTime = formatTime(selectedTime);

    const body = `Hi guys, Lets connect on ${formattedDate} at ${formattedTime} for 30 mins to discuss further about the product from your company ${companyText}. Thank you.`;
    const subject = `Meeting Request - ${companyText}`;
  ...
  const renderDirectory = () => {
    ...
    if (directory.every(company => company.founders.length === 0)) {
    ...
    return (
      <div className="space-y-6">
        {directory.map(company => (
          <div key={company.companyName} className="space-y-3">
            <h3 className="text-lg font-semibold text-primary">{company.companyName}</h3>
